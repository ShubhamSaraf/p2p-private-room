...
The project is being implemented one tested phase at a time. Phase 7 adds encrypted image and arbitrary-file offers, accepted binary chunk transfer with backpressure, optional local ZIP compression, and mutual SHA-256 integrity confirmation. Cloudflare still handles only room creation and WebRTC signaling. See the Phase 5–7 records in `docs/` for acceptance details.
...
- `packages/transfer` — binary chunk framing, backpressure, ZIP, and integrity helpers.
